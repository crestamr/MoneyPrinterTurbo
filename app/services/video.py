...
def render_image_as_zoom_clip(image_path: str, duration: float) -> str:
    ...
    Thin keyword-compatible wrapper over ``render_image_zoom_video`` kept for
    the Qwen-Image provider, which passes a float ``duration``. Returns the
    path to the written .mp4 file (image_path with ".mp4" appended).
    """
    return render_image_zoom_video(image_path, duration)
